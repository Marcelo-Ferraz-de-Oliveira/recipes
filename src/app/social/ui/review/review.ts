...
export class Review implements OnChanges{
  ...
  ngOnChanges(): void {
    ...
    if (comments) {
      this.rating = comments.length <=0 ? 0 : comments.reduce((acc, comment) => acc + comment.rating, 0,)/comments.length;
      for (let i = 5; i>0; i--) {
        this.map.set(i, comments.filter((comment) => comment.rating === i).length,);
      }
    }
  }
